...
result_dir <- file.path("outputs", "restricted", "real_data")
figure_dir <- file.path("outputs", "figures")
dir.create(figure_dir, recursive = TRUE, showWarnings = FALSE)

Gaussian_Test_Results <- readRDS(file.path(result_dir, "Gaussian_Test_Results.rds"))%>%mutate(Type="Gaussian")
...
G1$`Score Test`=G2$p
Binary_Test_Results <- readRDS(file.path(result_dir, "Binary_Test_Results.rds"))%>%mutate(Type="Binary")
...
svg(file.path(figure_dir, "manhatten.svg"),width=13,height=5.5)
...
Test_Results <- readRDS(file.path(result_dir, "Gaussian_Test_Results_quantile.rds"))%>%mutate(Type="Gaussian")
...
pdf(file.path(figure_dir, "manhatten_quantile.pdf"),width=11,height=5.5)
ggplot(score_df, aes(x = Trait, y = log10P, color = Category)) +
  geom_segment(aes(xend = Trait, y = 0, yend = log10P), linewidth = 1.2) +
  geom_segment(aes(xend = Trait, y = 0, yend = log10P), linewidth = 0.2, color = "black") +
  geom_point(size = 2.8) +
