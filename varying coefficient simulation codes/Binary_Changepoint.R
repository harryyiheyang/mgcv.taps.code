...
out_dir <- file.path("outputs", "varying_coefficient_simulation")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
filename <- file.path(out_dir, "Binary_Changepoint.rds")
for(i in 1:5){
for(j in 1:10){
...
f4=0*x4
eta=f1*z1+f2*z2+f3*z3
eta=eta-mean(eta)
y=rbinom(n,1,exp(eta+0.5)/(1+exp(eta+0.5)))
fit=gam(y~s(x1,bs="AMatern",by=z1,k=10,m=100,xt=list(getA=piecewise_linearity,para=0.5))+s(x2,by=z2,bs="cr",k=10)+
          s(x3,by=z3,bs="cr",k=15)+s(x4,by=z4,bs="cr",k=10),method="REML",family=binomial())
