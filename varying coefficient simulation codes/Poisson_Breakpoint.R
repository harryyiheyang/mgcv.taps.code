...
filename <- file.path(out_dir, "Poisson_Breakpoint.rds")
for(i in 1:5){
for(j in 1:10){
...
eta=eta-mean(eta)+1
y=rpois(n,exp(eta))
fit=gam(y~s(x1,by=z1,bs="AMatern",k=10,m=100,xt=list(getA=linearity_discontinuity,para=0.5))+s(x2,by=z2,bs="cr",k=10)+
          s(x3,by=z3,bs="cr",k=15)+s(x4,by=z4,bs="cr",k=10),method="REML",family=poisson())
