...
filename <- file.path(out_dir, "Binary_Breakpoint.rds")
for(i in 1:5){
for(j in 1:10){
...
eta=eta-mean(eta)+0.5
y=rbinom(n,1,exp(eta)/(1+exp(eta)))
fit=bam(y~s(x1,by=z1,bs="AMatern",k=10,m=100,xt=list(getA=linearity_discontinuity,para=0.5))+s(x2,by=z2,bs="cr",k=10)+
