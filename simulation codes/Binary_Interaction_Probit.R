...
filename <- file.path(out_dir, "Binary_Interaction_Probit.rds")
for(i in 1:5){
for(j in 1:10){
...
eta=f12+f3+f4
y=rbinom(n,1,pnorm((eta-5)/sqrt(3.2)))
fit=gam(y~s(x1,x2,bs="A2Matern",k=10,m=100)+s(x3,bs="cr",k=10)+s(x4,bs="cr",k=15),method="REML",family=binomial(link="probit"))
