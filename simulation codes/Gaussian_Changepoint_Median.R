...
out_dir <- file.path("outputs", "simulation")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
filename <- file.path(out_dir, "Gaussian_Changepoint_Median.rds")
for(i in 1:5){
for(j in 1:10){
...
fit1=gam(y~s(x1,bs="AMatern",k=10,m=100,xt=list(getA=piecewise_linearity,para=0.5))+s(x2,bs="cr",k=10)+s(x3,bs="cr",k=15)+s(x4,bs="cr",k=10),method="REML")
test1=taps_wald_test(fit1,test.component=1)
dat=data.frame(y=y,x1=x1,x2=x2,x3=x3,x4=x4)
fit2=qgam(y~s(x1,bs="AMatern",k=10,m=100,xt=list(getA=piecewise_linearity,para=0.5))+s(x2,bs="cr",k=10)+s(x3,bs="cr",k=15)+s(x4,bs="cr",k=10),qu=0.5,data=dat)
