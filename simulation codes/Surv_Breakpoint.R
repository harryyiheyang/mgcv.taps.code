...
filename <- file.path(out_dir, "Surv_Breakpoint.rds")
for(i in 1:5){
for(j in 1:10){
...
fit <- gam(Surv(time, status) ~ s(x1,bs="AMatern",k=10,m=100,xt=list(getA=linearity_discontinuity,para=0.5))
         +s(x2, bs="cr", k=10)+s(x3, bs="cr", k=15)+s(x4, bs="cr", k=10),
         data = data,
         method = "REML",
         family = cox.ph())

test=taps_wald_test(fit,test.component=1)
