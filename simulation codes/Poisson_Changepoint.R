...
filename <- file.path(out_dir, "Poisson_Changepoint.rds")
for(i in 1:5){
for(j in 1:10){
...
eta=eta/sd(eta)
y=rpois(n,exp(eta))
fit=gam(y~s(x1,bs="AMatern",k=10,m=100,xt=list(getA=piecewise_linearity,para=0.5))+s(x2,bs="cr",k=10)+s(x3,bs="cr",k=15)+s(x4,bs="cr",k=10),method="REML",family=poisson())
