...
filename <- file.path(out_dir, "Binary_Changepoint_Probit.rds")
for(i in 1:5){
for(j in 1:10){
...
f4=0*x4
eta=f1+f2+f3
y=rbinom(n,1,pnorm((eta-2)/sqrt(3.2)))
fit=gam(y~s(x1,bs="AMatern",k=10,m=100,xt=list(getA=piecewise_linearity,para=0.5))+s(x2,bs="cr",k=10)+s(x3,bs="cr",k=15)+s(x4,bs="cr",k=10),method="REML",family=binomial(link="probit"))
