...
for (i in seq_along(NAM_Disease)) {
trait <- NAM_Disease[i]
...
t1 <- taps_wald_test(fit)[,6:8] %>% dplyr::select(p=smooth.pvalue)
t2 <- taps_score_test(fit) %>% dplyr::select(p=smooth.pvalue)
t1$Test="Wald Test"
t2$Test="Score Test"
t=rbind(t1,t2)
t$Trait=unname(disease_fullnames[trait])
test_results[[i]]=t
...
test=anova(fit1,fit)
g_Outcome=data.frame(outcome=trait,int.est=coef(fit)[glue("s(Age):{paste0(trait,'_PRS')}.2")],int.se=summary(fit)$se[glue("s(Age):{paste0(trait,'_PRS')}.2")],wald.p=t1$p,score.p=t2$p)
G[[i]]=g_Outcome
