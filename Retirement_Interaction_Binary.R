...
w2 <- readRDS("~/mgcv_maps_simulation/UKBB_Disease.rds")%>%dplyr::select(ID,CAD,Stroke,HF,AF,PAD,T2D,SMK,BPDrug,paste0("PC",1:10))
...
NAM_Disease=NAM_Disease[2:8]
NAM_Disease=c(NAM_Disease,"DRNK")

w1=dplyr::filter(w1,Age>63&Age<67)
...
fit_Outcome=bam(formula_Outcome,method="REML",discrete=F,data=wi,family=binomial())
...
G1=do.call(rbind,G1)
G2=do.call(rbind,G2)
write.csv(G1,"~/mgcv_taps_varying/Binary_Exposure_RDD.csv")
write.csv(G2,"~/mgcv_taps_varying/Binary_Exposure_RKD.csv")
